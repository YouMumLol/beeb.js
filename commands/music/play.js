...
module.exports = {
  ...
  async autocomplete(interaction) {
    ...
    if (focusedValue && focusedValue.length > 0) {
      ...
      const searchResults = await yts(focusedValue); // This takes time, might block execution
      console.log("finished search: ", focusedValue);
      const choices = searchResults.videos
        .slice(0, 5)
        .map((video) => ({ name: video.title, value: video.url }));
  ...
  async execute(interaction) {
    ...
    if (!serverQueue) {
      ...
      await interaction.editReply(
        `🎶 | Added **${metadata.videoDetails.title}** to the queue.`
      );
    } else {
      const metadata = await ytdl.getBasicInfo(url);
      serverQueue.queue.push(metadata);
